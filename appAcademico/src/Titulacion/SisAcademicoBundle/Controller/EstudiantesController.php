                ...
                if($session->get('perfil') == $perfilEst || $session->get('perfil') == $perfilEstDoc || $session->get('perfil') == $perfilEstAdm){
                   ...
                   $asistencia = array();
                    try
                       {
                        $UgServices = new UgServices;
                        $xml = $UgServices->getConsultaAlumno_Asistencia($idCarrera,$idEstudiante,$ciclo,$anio);
                        if ( is_object($xml))
                        {
                          foreach($xml->registros->registros as $lcAsistencia) 
                            {

                              $valAsistencia= (double) $lcAsistencia->Porcentaje*100;
